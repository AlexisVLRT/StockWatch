...
def get_positions(db_id, tickers):
    ...
    cursor = conn.cursor()
    cursor.execute("""SELECT Ticker, LongPosition, Invested, ShortPosition, Provisioned, LongID, ShortID, LongDiff, ShortDiff FROM CurrentPositions{} WHERE Ticker IN {}""".format(db_id, tickers))
...
def update_position(db_id, ticker, long, invested, short, provisioned, long_id, short_id, long_diff, short_diff):
    conn = mysql.connector.connect(host=dc.host, user=dc.user, password=dc.password, database=dc.database)
    cursor = conn.cursor()
    cursor.execute("""UPDATE CurrentPositions{} SET  LongPosition = '{}', Invested = '{}', LongID = '{}', LongDiff = '{}', ShortPosition = '{}', Provisioned = '{}', ShortID = '{}', ShortDiff = '{}' WHERE Ticker = '{}'""".format(db_id, long, invested, long_id, long_diff, short, provisioned, short_id, short_diff, ticker))
    conn.commit()
    cursor.close()
    conn.close()


def open_position(db_id, trade_id, ticker, position, entry_date, entry_price, entry_money):
    conn = mysql.connector.connect(host=dc.host, user=dc.user, password=dc.password, database=dc.database)
    cursor = conn.cursor()
    cursor.execute("""INSERT INTO PositionsLog{} (Ticker, Position, EntryDate, EntryPrice, EntryMoney, TradeID) VALUES ('{}', '{}', '{}', '{}', '{}', '{}')""".format(db_id, ticker, position, entry_date, entry_price, entry_money, trade_id))
    conn.commit()
    cursor.close()
    conn.close()
...
def reset(db_id, tickers):
    assert (type(tickers) is list)
    tickers = tuple(tickers)

    conn = mysql.connector.connect(host=dc.host, user=dc.user, password=dc.password, database=dc.database)
    cursor = conn.cursor()
    cursor.execute("""UPDATE CurrentPositions{} SET  LongPosition = '0', Invested = '0', LongID=NULL, LongDiff=NULL, ShortPosition = '0', Provisioned = '0', ShortID=NULL, ShortDiff=NULL WHERE Ticker IN {}""".format(db_id, tickers))
    cursor.execute("""DELETE FROM PositionsLog{}""".format(db_id))
    conn.commit()
    cursor.close()
    conn.close()
...
if __name__ == '__main__':
    ...
    db_id = 1
    reset(db_id, tickers)
    print(get_positions(db_id, tickers))