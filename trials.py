...
hist_data = hist_data[~hist_data.index.duplicated(keep='last')].iloc[10946:]
...
diff_ = (diff.fillna(0).diff().ewm(span=0.5 * 390 // period).mean()).fillna(0) * 200
...
ax2.fill_between(list(diff2.reset_index(drop=True).index), (diff2 + diff3).fillna(0).values, alpha=0.5)
